...
export function ScrollArea({feedRecipes, header}:scrollAreaProps) {
  ...
  return (
    <div className='items-center justify-center flex flex-col gap-6 lg:h-[900px] md:h-[700px] h-[600px] bg-slate-100 dark:bg-darkBg1-200'>
...
export function RecipeBox({recipeName, recipeAuthor, recipeImage, recipeDescription, recipeId, recipeRating}: RecipeCardProps) {
  return (
    <Link className='snap-center flex flex-col justify-center  items-center md:gap-10 gap-16  lg:w-72 w-44 max-w-7xl rounded-lg hover:scale-110 hover:shadow-lg dark:hover:shadow-cyan-200 hover:shadow-orange-100 transition-transform hover:overflow-visible'
      href={`/recipes/${recipeId}`}>
      <Image src={recipeImage} alt={recipeName} width={300} height={500} className='rounded-lg lg:h-80 lg:w-72  object-cover sm:h-40 sm:w-36 dark:brightness-90 hover:dark:brightness-100' />
